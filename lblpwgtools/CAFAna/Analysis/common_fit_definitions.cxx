...
namespace {
  bool IsSplitXSecSystName(const std::string &name);
  std::string GetSplitXSecBaseName(const std::string &name);

  std::vector<std::string>
  GetFitParameterNames(const std::vector<const IFitVar *> &oscVars,
                       const std::vector<const ISyst *> &systlist) {
    std::vector<std::string> names;
    names.reserve(oscVars.size() + systlist.size());
  ...
  std::vector<double>
  GetFitParameterValues(const std::vector<const IFitVar *> &oscVars,
                        const std::vector<const ISyst *> &systlist,
                        osc::IOscCalculatorAdjustable *fitOsc,
                        const SystShifts &fitSyst) {
    std::vector<double> vals;
    vals.reserve(oscVars.size() + systlist.size());

    for (const IFitVar *v : oscVars)
      vals.push_back(v->GetValue(fitOsc));
    for (const ISyst *s : systlist)
      vals.push_back(fitSyst.GetShift(s));

    return vals;
  }

  void ApplyFitParameterValues(const std::vector<const IFitVar *> &oscVars,
                               const std::vector<const ISyst *> &systlist,
                               const std::vector<double> &vals,
                               osc::IOscCalculatorAdjustable *fitOsc,
                               SystShifts &fitSyst) {
    assert(vals.size() == oscVars.size() + systlist.size());

    for (size_t i = 0; i < oscVars.size(); ++i)
      oscVars[i]->SetValue(fitOsc, vals[i]);

    for (size_t j = 0; j < systlist.size(); ++j)
      fitSyst.SetShift(systlist[j], vals[oscVars.size() + j]);
  }

  double EvaluatePrefitPenalty(const std::vector<const IFitVar *> &oscVars,
                               const std::vector<const ISyst *> &systlist,
                               const std::vector<double> &vals,
                               osc::IOscCalculatorAdjustable *fitOsc,
                               SystShifts fitSyst,
                               IExperiment *penaltyTerm) {
    ApplyFitParameterValues(oscVars, systlist, vals, fitOsc, fitSyst);

    double chi = 0;
    for (size_t i = 0; i < oscVars.size(); ++i)
      chi += oscVars[i]->Penalty(vals[i], fitOsc);

    for (size_t j = 0; j < systlist.size(); ++j)
      chi += systlist[j]->Penalty(vals[oscVars.size() + j]);

    if (penaltyTerm)
      chi += penaltyTerm->ChiSq(fitOsc, fitSyst);

    return chi;
  }

  std::vector<double>
  GetPrefitPenaltySteps(const std::vector<double> &vals,
                        const std::vector<const IFitVar *> &oscVars,
                        const std::vector<const ISyst *> &systlist) {
    std::vector<double> steps;
    steps.reserve(vals.size());

    for (size_t i = 0; i < oscVars.size(); ++i) {
      const double val = vals[i];
      const double fit_step = val ? fabs(val / 2) : 0.1;
      steps.push_back(std::max(1e-5, fit_step * 1e-3));
    }

    for (size_t j = 0; j < systlist.size(); ++j)
      steps.push_back(1e-4);

    return steps;
  }

  std::vector<int>
  GetParameterSubsetIndices(const std::vector<std::string> &names,
                            bool wantXSec) {
    static const std::set<std::string> xsecNames(GetAllXSecSystNames().begin(),
                                                 GetAllXSecSystNames().end());

    std::vector<int> indices;
    for (size_t i = 0; i < names.size(); ++i) {
      const bool isXSec =
          xsecNames.count(names[i]) || IsSplitXSecSystName(names[i]);
      if (isXSec == wantXSec)
        indices.push_back(i);
    }
    return indices;
  ...
  bool IsSplitXSecSystName(const std::string &name) {
    ...
    if (EndsWith(name, "_ND")) {
      return xsecNames.count(name.substr(0, name.size() - 3));
    }
    if (EndsWith(name, "_FD")) {
      return xsecNames.count(name.substr(0, name.size() - 3));
    }
    return false;
  }

  bool IsBaseXSecSystName(const std::string &name) {
    static const std::set<std::string> xsecNames(GetAllXSecSystNames().begin(),
                                                 GetAllXSecSystNames().end());
    return xsecNames.count(name);
  }

  const ISyst *GetDetectorSplitXSecSyst(const std::string &baseName, bool useFD,
                                        bool applyPenalty) {
    const std::vector<const ISyst *> splitSysts =
        GetSplitXSecSysts({baseName}, applyPenalty);
    const std::string suffix = useFD ? "_FD" : "_ND";
    for (const ISyst *s : splitSysts) {
      if (EndsWith(s->ShortName(), suffix)) {
        return s;
      }
    }
    return nullptr;
  ...
  const ISyst *GetBasePredictionXSecSyst(const std::string &baseName) {
    static std::vector<const ISyst *> allPredictionSysts = GetListOfSysts();
    for (const ISyst *s : allPredictionSysts) {
      if (s->ShortName() == baseName) {
        return s;
      }
    }
    return nullptr;
  ...
  SystShifts MakeSplitPredictionSystShifts(const SystShifts &input) {
    SystShifts output = kNoShift;

    for (const ISyst *s : input.ActiveSysts()) {
      const double shift = input.GetShift(s);
      const std::string shortName = s->ShortName();

      if (IsSplitXSecSystName(shortName) || !IsBaseXSecSystName(shortName)) {
        output.SetShift(s, shift);
        continue;
      }

      const ISyst *ndSyst =
          GetDetectorSplitXSecSyst(shortName, false, s->ApplyPenalty());
      const ISyst *fdSyst =
          GetDetectorSplitXSecSyst(shortName, true, s->ApplyPenalty());

      if (ndSyst)
        output.SetShift(ndSyst, shift);
      if (fdSyst)
        output.SetShift(fdSyst, shift);
    }

    return output;
  }

  SystShifts MakeBasePredictionSystShifts(const SystShifts &input, bool useFD) {
    SystShifts output = kNoShift;
    ...
    for (const ISyst *s : input.ActiveSysts()) {
      ...
      if (IsSplitXSecSystName(shortName)) {
        const std::string baseName = GetSplitXSecBaseName(shortName);
        const bool isFDShift = EndsWith(shortName, "_FD");
        if (isFDShift != useFD) {
          continue;
        }

        const ISyst *baseSyst = GetBasePredictionXSecSyst(baseName);
        if (baseSyst) {
          output.SetShift(baseSyst, shift);
        }
        continue;
      }
  ...
  std::vector<std::pair<const ISyst *, const ISyst *>>
  GetSharedToSplitXSecCorrelations(const std::vector<const ISyst *> &systlist,
                                   bool useFD) {
    std::vector<std::pair<const ISyst *, const ISyst *>> corrs;

    for (const ISyst *s : systlist) {
      const std::string shortName = s->ShortName();
      if (!IsBaseXSecSystName(shortName))
        continue;

      const ISyst *splitSyst =
          GetDetectorSplitXSecSyst(shortName, useFD, s->ApplyPenalty());
      if (splitSyst) {
        corrs.emplace_back(s, splitSyst);
      }
    }
  ...
  std::vector<std::pair<const ISyst *, const ISyst *>>
  GetFitToBasePredictionXSecCorrelations(const std::vector<const ISyst *> &systlist,
                                         bool useFD) {
    std::vector<std::pair<const ISyst *, const ISyst *>> corrs;
    ...
    for (const ISyst *s : systlist) {
      ...
      const bool isFDShift = EndsWith(shortName, "_FD");
      const ISyst *baseSyst = GetBasePredictionXSecSyst(baseName);
      corrs.emplace_back(s, (isFDShift == useFD) ? baseSyst : nullptr);
    }
  ...
  ExpandSplitXSecSysts(const std::vector<const ISyst *> &inputList) {
    static const std::set<std::string> xsecNames(GetAllXSecSystNames().begin(),
                                                 GetAllXSecSystNames().end());

    std::vector<const ISyst *> splitList;
    splitList.reserve(inputList.size() * 2);
    for (const ISyst *s : inputList) {
      if (!xsecNames.count(s->ShortName())) {
        splitList.push_back(s);
        continue;
      }

      std::vector<const ISyst *> splitSysts =
          GetSplitXSecSysts({s->ShortName()}, s->ApplyPenalty());
      splitList.insert(splitList.end(), splitSysts.begin(), splitSysts.end());
    }

    return splitList;
  }

  bool ShouldSplitXSecSyst(const std::string &baseName,
                           const SplitXSecCorrelationConfig &config) {
    static const std::set<std::string> xsecNames(GetAllXSecSystNames().begin(),
                                                 GetAllXSecSystNames().end());

    if (!xsecNames.count(baseName))
      return false;

    if (std::find(config.dontSplitXSecNames.begin(),
                  config.dontSplitXSecNames.end(),
                  baseName) != config.dontSplitXSecNames.end()) {
      return false;
    }

    if (config.splitAllXSec)
      return true;

    return std::find(config.splitXSecNames.begin(), config.splitXSecNames.end(),
                     baseName) != config.splitXSecNames.end();
  }

  std::vector<const ISyst *>
  NormalizeSplitXSecSysts(const std::vector<const ISyst *> &inputList,
                          const SplitXSecCorrelationConfig &config) {
    static const std::set<std::string> xsecNames(GetAllXSecSystNames().begin(),
                                                 GetAllXSecSystNames().end());

    std::vector<const ISyst *> outputList;
    ...
    for (const ISyst *s : inputList) {
      ...
      if (IsSplitXSecSystName(shortName)) {
        if (usedNames.insert(shortName).second) {
          outputList.push_back(s);
        }
        continue;
      }

      if (!xsecNames.count(shortName) ||
          !ShouldSplitXSecSyst(shortName, config)) {
        if (usedNames.insert(shortName).second) {
          outputList.push_back(s);
        }
        continue;
      ...
      const std::vector<const ISyst *> splitSysts =
          GetSplitXSecSysts({shortName}, s->ApplyPenalty());
      for (const ISyst *splitSyst : splitSysts) {
        if (usedNames.insert(splitSyst->ShortName()).second) {
          outputList.push_back(splitSyst);
        }
      }
    }
  ...
  std::string GetSplitXSecBaseName(const std::string &name) {
    if (!IsSplitXSecSystName(name))
      return "";
    return name.substr(0, name.size() - 3);
  }

  double GlobalRhoToDifferencePenaltyStrength(double rho) {
    const double boundedRho = std::max(0.0, std::min(rho, 1.0 - 1e-6));
    if (boundedRho == 0)
      return 0;

    // Keep the usual unit-width nuisance penalties and add
    // lambda * (x_ND - x_FD)^2. This corresponds to a marginal correlation
    // coefficient rho = lambda / (1 + lambda), so lambda = rho / (1 - rho).
    return boundedRho / (1.0 - boundedRho);
  }

  double GetSplitXSecCorrelationRho(const std::string &baseName,
                                    const SplitXSecCorrelationConfig &config) {
    const auto mapIt = config.splitXSecCorrByName.find(baseName);
    if (mapIt != config.splitXSecCorrByName.end()) {
      return mapIt->second;
    }

    const size_t nOverrides =
        std::min(config.splitXSecCorrNames.size(),
                 config.splitXSecCorrStrengths.size());

    for (size_t i = 0; i < nOverrides; ++i) {
      if (config.splitXSecCorrNames[i] == baseName) {
        return config.splitXSecCorrStrengths[i];
      }
    }

    return config.globalRho;
  }

  struct SplitXSecCorrelationPair {
    const ISyst *ndSyst = nullptr;
    const ISyst *fdSyst = nullptr;
    double rho = 0;
  };

  std::vector<SplitXSecCorrelationPair>
  GetSplitXSecCorrelationPairs(const std::vector<const ISyst *> &systlist,
                               const SplitXSecCorrelationConfig &config) {
    std::map<std::string, std::pair<const ISyst *, const ISyst *>> splitPairs;

    for (const ISyst *s : systlist) {
      const std::string name = s->ShortName();
      const std::string baseName = GetSplitXSecBaseName(name);
      if (baseName.empty())
        continue;

      auto &pair = splitPairs[baseName];
      if (EndsWith(name, "_ND")) {
        pair.first = s;
      } else if (EndsWith(name, "_FD")) {
        pair.second = s;
      }
    }

    std::vector<SplitXSecCorrelationPair> pairs;
    pairs.reserve(splitPairs.size());
    for (const auto &entry : splitPairs) {
      if (entry.second.first && entry.second.second) {
        pairs.push_back({entry.second.first, entry.second.second,
                         GetSplitXSecCorrelationRho(entry.first, config)});
      }
    }
  ...
  class SplitXSecCorrelationPenalty : public IExperiment {
  public:
    explicit SplitXSecCorrelationPenalty(std::vector<SplitXSecCorrelationPair> pairs)
        : fPairs(std::move(pairs)) {}

    bool IsActive() const {
      return std::any_of(fPairs.begin(), fPairs.end(),
                         [](const SplitXSecCorrelationPair &pair) {
                           return GlobalRhoToDifferencePenaltyStrength(pair.rho) > 0;
                         });
    }

    double ChiSq(osc::IOscCalculatorAdjustable *,
                 const SystShifts &syst = SystShifts::Nominal()) const override {
      double chi = 0;
      for (const auto &pair : fPairs) {
        const double strength =
            GlobalRhoToDifferencePenaltyStrength(pair.rho);
        if (strength == 0)
          continue;

        const double diff =
            syst.GetShift(pair.ndSyst) - syst.GetShift(pair.fdSyst);
        chi += strength * diff * diff;
      }
      return chi;
    }
    ...
    void Derivative(osc::IOscCalculator *,
      ...
      for (const auto &pair : fPairs) {
        ...
        const double diff =
            syst.GetShift(pair.ndSyst) - syst.GetShift(pair.fdSyst);
        dchi[pair.ndSyst] += 2 * strength * diff;
        dchi[pair.fdSyst] -= 2 * strength * diff;
      }
    }

  private:
    std::vector<SplitXSecCorrelationPair> fPairs;
  };

  TH2D MakeLabeledMatrixHist(const TMatrixD &mat,
                             const std::vector<std::string> &names,
                             const std::string &histName,
                             const std::string &histTitle) {
    const int n = mat.GetNrows();
    TH2D hist(histName.c_str(), histTitle.c_str(), n, 0, n, n, 0, n);
    for (int i = 0; i < n; ++i) {
      if (names.size() == size_t(n)) {
        hist.GetXaxis()->SetBinLabel(i + 1, names[i].c_str());
        hist.GetYaxis()->SetBinLabel(i + 1, names[i].c_str());
      }
      for (int j = 0; j < n; ++j) {
  ...
  TH2D MakeSubmatrixHist(const TMatrixD &mat,
                         const std::vector<std::string> &names,
    ...
    for (int i = 0; i < n; ++i) {
      ...
      hist.GetXaxis()->SetBinLabel(i + 1, names[row].c_str());
      hist.GetYaxis()->SetBinLabel(i + 1, names[row].c_str());
      for (int j = 0; j < n; ++j) {
        const int col = indices[j];
        hist.SetBinContent(i + 1, j + 1, mat(row, col));
      }
    }
    return hist;
  }

  TMatrixD HistToMatrix(const TH2D &hist) {
  ...
  void WriteCovarianceSubsets(const TMatrixD &mat,
                              const std::vector<std::string> &names,
                              const std::string &baseName) {
    const std::vector<int> xsecIdx = GetParameterSubsetIndices(names, true);
    const std::vector<int> otherIdx = GetParameterSubsetIndices(names, false);

    if (!xsecIdx.empty()) {
      TH2D xsecCov = MakeSubmatrixHist(mat, names, xsecIdx, baseName + "_xsec",
                                       baseName + " xsec block");
      std::unique_ptr<TH2D> xsecCorr(make_corr_from_covar(&xsecCov));
      xsecCorr->SetNameTitle((baseName + "_xsec_corr").c_str(),
                             (baseName + " xsec correlation").c_str());
      xsecCov.Write();
      xsecCorr->Write();
    }
    ...
    if (!otherIdx.empty()) {
      TH2D otherCov =
          MakeSubmatrixHist(mat, names, otherIdx, baseName + "_other",
                            baseName + " non-xsec block");
      std::unique_ptr<TH2D> otherCorr(make_corr_from_covar(&otherCov));
      otherCorr->SetNameTitle((baseName + "_other_corr").c_str(),
                              (baseName + " non-xsec correlation").c_str());
      otherCov.Write();
      otherCorr->Write();
    }
  }

  void WritePrefitPenaltyMatrix(
      TDirectory *outDir, const std::vector<const IFitVar *> &oscVars,
      const std::vector<const ISyst *> &systlist,
    ...
    const std::vector<std::string> names =
        GetFitParameterNames(oscVars, systlist);
    const std::vector<double> x0 =
        GetFitParameterValues(oscVars, systlist, fitOsc, fitSyst);
    const std::vector<double> steps =
        GetPrefitPenaltySteps(x0, oscVars, systlist);

    const int n = names.size();

    TH1D param_vals("prefit_param_values", "Prefit parameter values", n, 0, n);
    TH2D hess("prefit_penalty_hessian",
              "Prefit penalty Hessian in fit-parameter basis", n, 0, n, n, 0,
              n);
    TH2D prec("prefit_penalty_precision",
              "Prefit penalty precision in fit-parameter basis", n, 0, n, n, 0,
              n);

    for (int i = 0; i < n; ++i) {
      param_vals.GetXaxis()->SetBinLabel(i + 1, names[i].c_str());
      param_vals.SetBinContent(i + 1, x0[i]);
      hess.GetXaxis()->SetBinLabel(i + 1, names[i].c_str());
      hess.GetYaxis()->SetBinLabel(i + 1, names[i].c_str());
      prec.GetXaxis()->SetBinLabel(i + 1, names[i].c_str());
      prec.GetYaxis()->SetBinLabel(i + 1, names[i].c_str());
    }

    const std::vector<double> baseline = x0;

    for (int i = 0; i < n; ++i) {
      const double hi = steps[i];

      std::vector<double> xp = baseline;
      std::vector<double> xm = baseline;
      xp[i] += hi;
      xm[i] -= hi;

      const double fp = EvaluatePrefitPenalty(oscVars, systlist, xp, fitOsc,
                                              fitSyst, penaltyTerm);
      const double f0 = EvaluatePrefitPenalty(oscVars, systlist, baseline,
                                              fitOsc, fitSyst, penaltyTerm);
      const double fm = EvaluatePrefitPenalty(oscVars, systlist, xm, fitOsc,
                                              fitSyst, penaltyTerm);

      const double hii = (fp - 2 * f0 + fm) / (hi * hi);
      hess.SetBinContent(i + 1, i + 1, hii);
      prec.SetBinContent(i + 1, i + 1, 0.5 * hii);

      for (int j = i + 1; j < n; ++j) {
        const double hj = steps[j];

        std::vector<double> xpp = baseline;
        std::vector<double> xpm = baseline;
        std::vector<double> xmp = baseline;
        std::vector<double> xmm = baseline;
        ...
        const double fpp = EvaluatePrefitPenalty(oscVars, systlist, xpp,
                                                 fitOsc, fitSyst, penaltyTerm);
        const double fpm = EvaluatePrefitPenalty(oscVars, systlist, xpm,
                                                 fitOsc, fitSyst, penaltyTerm);
        const double fmp = EvaluatePrefitPenalty(oscVars, systlist, xmp,
                                                 fitOsc, fitSyst, penaltyTerm);
        const double fmm = EvaluatePrefitPenalty(oscVars, systlist, xmm,
                                                 fitOsc, fitSyst, penaltyTerm);

        const double hij = (fpp - fpm - fmp + fmm) / (4 * hi * hj);
        hess.SetBinContent(i + 1, j + 1, hij);
        hess.SetBinContent(j + 1, i + 1, hij);
        prec.SetBinContent(i + 1, j + 1, 0.5 * hij);
        prec.SetBinContent(j + 1, i + 1, 0.5 * hij);
      }
    }

    ApplyFitParameterValues(oscVars, systlist, baseline, fitOsc, fitSyst);

    outDir->cd();
    param_vals.Write();
    hess.Write();
    prec.Write();
    WriteCovarianceSubsets(HistToMatrix(prec), names, "prefit_penalty_precision");
    WriteCovarianceSubsets(HistToMatrix(hess), names, "prefit_penalty_hessian");
  }
} // namespace
...
std::vector<const ISyst *>
GetListOfSystsSplitXSec(bool fluxsyst_Nov17, bool xsecsyst, bool detsyst,
                        bool useND, bool useFD, bool useNueOnE,
                        bool useFakeDataDials, bool fluxsyst_CDR,
                        int NFluxSysts, bool removeFDNonFitDials) {
  return ExpandSplitXSecSysts(
      GetListOfSysts(fluxsyst_Nov17, xsecsyst, detsyst, useND, useFD,
                     useNueOnE, useFakeDataDials, fluxsyst_CDR, NFluxSysts,
                     removeFDNonFitDials));
}

std::vector<const ISyst *>
GetListOfSystsSplitXSec(std::string systString, bool useND, bool useFD,
                        bool useNueOnE) {
  return ExpandSplitXSecSysts(GetListOfSysts(systString, useND, useFD,
                                             useNueOnE));
}

std::vector<const ISyst *>
GetListOfSystsSplitXSec(char const *systCString, bool useND, bool useFD,
                        bool useNueOnE) {
  return GetListOfSystsSplitXSec(std::string(systCString), useND, useFD,
                                 useNueOnE);
}
...
std::vector<std::unique_ptr<ana::PredictionInterp>>
GetPredictionInterpsSplitXSec(std::string fileName,
                              std::vector<const ISyst *> systlist) {

  // Make sure the syst registry has been populated with all split xsec systs
  // needed to deserialize the PredictionInterp payloads for this path.
  static std::vector<const ISyst *> systs = GetListOfSystsSplitXSec();
  ...
  for (size_t s_it = 0; s_it < sample_dir_order.size(); ++s_it) {
    std::string state_fname =
        fileNameIsStub ? fileName + "_" + sample_suffix_order[s_it] + ".root"
                       : fileName;
    TFile *fin = TFile::Open(state_fname.c_str(), "READ");
    assert(fin && !fin->IsZombie());
    std::cout << "[LOAD]: Retrieving " << sample_dir_order[s_it] << " from "
              << state_fname << ":" << sample_dir_order[s_it] << std::endl;
    return_list.emplace_back(LoadFrom<PredictionInterp>(
...
double RunFitPoint(std::string stateFileName, std::string sampleString,
  ...
  std::vector<std::pair<std::string, std::unique_ptr<TMatrixD>>> usedCovMats;
  ...
  if (UseNDCovMat && (pot_nd_rhc > 0) && (pot_nd_fhc > 0)) {
    ...
    if (UseV3NDCovMat) {
      ...
      usedCovMats.emplace_back("nd_frac_cov_used",
                               std::make_unique<TMatrixD>(*fake_uncorr));
      ...
      this_expt.AddCovarianceMatrix(covFileName, "nd_all_frac_cov", true,
                                    {0, 1});
      TFile covMatFile(covFileName.c_str());
      TMatrixD *covToSave = (TMatrixD *)covMatFile.Get("nd_all_frac_cov");
      if (covToSave) {
        usedCovMats.emplace_back("nd_frac_cov_used",
                                 std::make_unique<TMatrixD>(*covToSave));
      }
    }
  }
  ...
  if (UseNDCovMat && (pot_nd_rhc > 0) && (pot_nd_fhc == 0)) {
    ...
    TFile covMatFile(covFileName.c_str());
    TMatrixD *covToSave = (TMatrixD *)covMatFile.Get("nd_rhc_frac_cov");
    if (covToSave) {
      usedCovMats.emplace_back("nd_rhc_frac_cov_used",
                               std::make_unique<TMatrixD>(*covToSave));
    }
  }
  ...
  if (UseNDCovMat && (pot_nd_rhc == 0) && (pot_nd_fhc > 0)) {
    ...
    TFile covMatFile(covFileName.c_str());
    TMatrixD *covToSave = (TMatrixD *)covMatFile.Get("nd_fhc_frac_cov");
    if (covToSave) {
      usedCovMats.emplace_back("nd_fhc_frac_cov_used",
                               std::make_unique<TMatrixD>(*covToSave));
    }
  }
  ...
  if (outDir) {
    ...
    WritePrefitPenaltyMatrix(outDir, oscVars, systlist, fitOsc, fitSyst,
                             penaltyTerm);
  }
  ...
  if (outDir) {
    ...
    // Save information
    TH2D hist_covar = MakeLabeledMatrixHist(*covar, fParamNames, "covar",
                                            "Postfit covariance");
    std::unique_ptr<TH2D> hist_corr(make_corr_from_covar(&hist_covar));
    hist_corr->SetNameTitle("corr", "Postfit correlation");
    hist_covar.Write();
    hist_corr->Write();
    covar->Write("covar_mat");
    WriteCovarianceSubsets(*covar, fParamNames, "covar");
    for (const auto &namedMat : usedCovMats) {
      namedMat.second->Write(namedMat.first.c_str());
      TH2D covHist =
          MakeLabeledMatrixHist(*namedMat.second, {}, namedMat.first + "_hist",
                                namedMat.first);
      covHist.SetName((namedMat.first + "_hist").c_str());
      covHist.SetTitle(namedMat.first.c_str());
      std::unique_ptr<TH2D> corrHist(make_corr_from_covar(&covHist));
      corrHist->SetName((namedMat.first + "_corr").c_str());
      corrHist->SetTitle((namedMat.first + " correlation").c_str());
      covHist.Write();
      corrHist->Write();
    }
  }
...
double RunFitPoint_ND_FD_Separation(
    std::string stateFileName, std::string sampleString,
    osc::IOscCalculatorAdjustable *fakeDataOsc, SystShifts fakeDataSyst,
    bool fakeDataStats, std::vector<const IFitVar *> oscVars,
    std::vector<const ISyst *> systlist,
    osc::IOscCalculatorAdjustable *fitOsc, SystShifts fitSyst,
    ana::SeedList oscSeeds, IExperiment *penaltyTerm,
    Fitter::Precision fitStrategy, TDirectory *outDir,
    FitTreeBlob *PostFitTreeBlob, std::vector<seeded_spectra> *spectra,
    SystShifts &bf, SplitXSecCorrelationConfig xsecCorrConfig) {
  systlist = NormalizeSplitXSecSysts(systlist, xsecCorrConfig);
  ...
  const std::vector<std::pair<const ISyst *, const ISyst *>> ndSystCorrs =
      GetFitToBasePredictionXSecCorrelations(systlist, false);
  const std::vector<std::pair<const ISyst *, const ISyst *>> fdSystCorrs =
      GetFitToBasePredictionXSecCorrelations(systlist, true);
  ...
  if (pot_nd_fhc > 0) {
    this_expt.Add(&nd_expt_fhc);
    this_expt.SetSystCorrelations(exptIdx++, ndSystCorrs);
  }
  if (pot_nd_rhc > 0) {
    this_expt.Add(&nd_expt_rhc);
    this_expt.SetSystCorrelations(exptIdx++, ndSystCorrs);
  ...
  if (!ndprefit && pot_fd_fhc_numu > 0) {
    this_expt.Add(&dis_expt_fhc);
    this_expt.SetSystCorrelations(exptIdx++, fdSystCorrs);
  }
  if (!ndprefit && pot_fd_rhc_numu > 0) {
    this_expt.Add(&dis_expt_rhc);
    this_expt.SetSystCorrelations(exptIdx++, fdSystCorrs);
  ...
  SplitXSecCorrelationPenalty splitXSecCorrPenalty(
      GetSplitXSecCorrelationPairs(systlist, xsecCorrConfig));

  MultiExperiment activePenaltyTerms;
  bool haveActivePenaltyTerms = false;
  if (!ndprefit && penaltyTerm) {
    activePenaltyTerms.Add(penaltyTerm);
    haveActivePenaltyTerms = true;
  }
  if (splitXSecCorrPenalty.IsActive()) {
    activePenaltyTerms.Add(&splitXSecCorrPenalty);
    haveActivePenaltyTerms = true;
  }

  if (haveActivePenaltyTerms) {
    this_expt.Add(&activePenaltyTerms);
  }

  if (outDir) {
    outDir->cd();
    const SystShifts fitSystForFDPrediction =
        MakeBasePredictionSystShifts(fitSyst, true);
    const SystShifts fitSystForNDPrediction =
        MakeBasePredictionSystShifts(fitSyst, false);
    ...
    if (pot_fd_fhc_nue > 0) {
      ...
      data_nue_fhc_hist->SetName("data_fd_nue_fhc");
      data_nue_fhc_hist->Write();
      TH1 *pre_fd_nue_fhc = GetMCSystTotal(&predFDNueFHC, fitOsc,
                                           fitSystForFDPrediction,
                                           "prefit_fd_nue_fhc", pot_fd_fhc_nue);
      pre_fd_nue_fhc->SetTitle(
          std::to_string(app_expt_fhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
      pre_fd_nue_fhc->Write();
    }
    if (pot_fd_fhc_numu > 0) {
      ...
      data_numu_fhc_hist->Write();
      TH1 *pre_fd_numu_fhc =
          GetMCSystTotal(&predFDNumuFHC, fitOsc, fitSystForFDPrediction,
                         "prefit_fd_numu_fhc",
                         pot_fd_fhc_numu);
      pre_fd_numu_fhc->SetTitle(
          std::to_string(dis_expt_fhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
      pre_fd_numu_fhc->Write();
    }
    if (pot_fd_rhc_nue > 0) {
      ...
      data_nue_rhc_hist->SetName("data_fd_nue_rhc");
      data_nue_rhc_hist->Write();
      TH1 *pre_fd_nue_rhc = GetMCSystTotal(&predFDNueRHC, fitOsc,
                                           fitSystForFDPrediction,
                                           "prefit_fd_nue_rhc", pot_fd_rhc_nue);
      pre_fd_nue_rhc->SetTitle(
          std::to_string(app_expt_rhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
      pre_fd_nue_rhc->Write();
    }
    if (pot_fd_rhc_numu > 0) {
      ...
      data_numu_rhc_hist->Write();
      TH1 *pre_fd_numu_rhc =
          GetMCSystTotal(&predFDNumuRHC, fitOsc, fitSystForFDPrediction,
                         "prefit_fd_numu_rhc",
                         pot_fd_rhc_numu);
      pre_fd_numu_rhc->SetTitle(
          std::to_string(dis_expt_rhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
      pre_fd_numu_rhc->Write();
    }
    if (pot_nd_fhc > 0) {
      ...
      TH1 *pre_nd_numu_fhc = GetMCSystTotal(&predNDNumuFHC, fitOsc,
                                            fitSystForNDPrediction,
                                            "prefit_nd_numu_fhc", pot_nd_fhc);
      pre_nd_numu_fhc->SetTitle(
          std::to_string(this_nd_fhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
      pre_nd_numu_fhc->Write();
      TH1 *pre_nd_numu_fhc_1D = GetMCSystTotal(
          &predNDNumuFHC, fitOsc, fitSystForNDPrediction,
          "prefit_nd_numu_fhc_1D", pot_nd_fhc,
          true);
      pre_nd_numu_fhc_1D->SetTitle(
          std::to_string(this_nd_fhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
    ...
    if (pot_nd_rhc) {
      ...
      TH1 *pre_nd_numu_rhc = GetMCSystTotal(&predNDNumuRHC, fitOsc,
                                            fitSystForNDPrediction,
                                            "prefit_nd_numu_rhc", pot_nd_rhc);
      pre_nd_numu_rhc->SetTitle(
          std::to_string(this_nd_rhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
      pre_nd_numu_rhc->Write();
      TH1 *pre_nd_numu_rhc_1D = GetMCSystTotal(
          &predNDNumuRHC, fitOsc, fitSystForNDPrediction,
          "prefit_nd_numu_rhc_1D", pot_nd_rhc,
          true);
      pre_nd_numu_rhc_1D->SetTitle(
          std::to_string(this_nd_rhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
    ...
    WritePrefitPenaltyMatrix(outDir, oscVars, systlist, fitOsc, fitSyst,
                             haveActivePenaltyTerms ? &activePenaltyTerms
                                                    : nullptr);
  }
  ...
  if (outDir) {
    ...
    if (pot_fd_fhc_nue) {
      TH1 *post_fd_nue_fhc = GetMCSystTotal(
          &predFDNueFHC, fitOsc, fitSystForFDPrediction, "postfit_fd_nue_fhc",
          pot_fd_fhc_nue);
      post_fd_nue_fhc->SetTitle(
          std::to_string(app_expt_fhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
    ...
    if (pot_fd_fhc_numu) {
      TH1 *post_fd_numu_fhc = GetMCSystTotal(
          &predFDNumuFHC, fitOsc, fitSystForFDPrediction, "postfit_fd_numu_fhc",
          pot_fd_fhc_numu);
      post_fd_numu_fhc->SetTitle(
          std::to_string(dis_expt_fhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
    ...
    if (pot_fd_rhc_nue) {
      TH1 *post_fd_nue_rhc = GetMCSystTotal(
          &predFDNueRHC, fitOsc, fitSystForFDPrediction, "postfit_fd_nue_rhc",
          pot_fd_rhc_nue);
      post_fd_nue_rhc->SetTitle(
          std::to_string(app_expt_rhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
    ...
    if (pot_fd_rhc_numu) {
      TH1 *post_fd_numu_rhc = GetMCSystTotal(
          &predFDNumuRHC, fitOsc, fitSystForFDPrediction, "postfit_fd_numu_rhc",
          pot_fd_rhc_numu);
      post_fd_numu_rhc->SetTitle(
          std::to_string(dis_expt_rhc.ChiSq(fitOsc, fitSystForFDPrediction))
              .c_str());
    ...
    if (pot_nd_fhc) {
      TH1 *post_nd_numu_fhc = GetMCSystTotal(&predNDNumuFHC, fitOsc,
                                             fitSystForNDPrediction,
                                             "postfit_nd_numu_fhc", pot_nd_fhc);
      post_nd_numu_fhc->SetTitle(
          std::to_string(this_nd_fhc.ChiSq(fitOsc, fitSystForNDPrediction))
      ...
      post_nd_numu_fhc->Write();
      TH1 *post_nd_numu_fhc_1D =
          GetMCSystTotal(&predNDNumuFHC, fitOsc, fitSystForNDPrediction,
                         "postfit_nd_numu_fhc_1D", pot_nd_fhc, true);
      post_nd_numu_fhc_1D->SetTitle(
          std::to_string(this_nd_fhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
    ...
    if (pot_nd_rhc) {
      TH1 *post_nd_numu_rhc = GetMCSystTotal(&predNDNumuRHC, fitOsc,
                                             fitSystForNDPrediction,
                                             "postfit_nd_numu_rhc", pot_nd_rhc);
      post_nd_numu_rhc->SetTitle(
          std::to_string(this_nd_rhc.ChiSq(fitOsc, fitSystForNDPrediction))
      ...
      post_nd_numu_rhc->Write();
      TH1 *post_nd_numu_rhc_1D =
          GetMCSystTotal(&predNDNumuRHC, fitOsc, fitSystForNDPrediction,
                         "postfit_nd_numu_rhc_1D", pot_nd_rhc, true);
      post_nd_numu_rhc_1D->SetTitle(
          std::to_string(this_nd_rhc.ChiSq(fitOsc, fitSystForNDPrediction))
              .c_str());
