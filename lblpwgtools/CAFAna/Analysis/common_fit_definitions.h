...
std::vector<const ana::ISyst *>
GetListOfSystsSplitXSec(bool fluxsyst_Nov17 = true, bool xsecsyst = true,
                        bool detsyst = true, bool useND = true,
                        bool useFD = true, bool useNueOnE = false,
                        bool useFakeDataDials = true,
                        bool fluxsyst_CDR = true,
                        int NFluxSysts = NFluxParametersToAddToStatefile,
                        bool removeFDNonFitDials = false);

std::vector<const ana::ISyst *>
GetListOfSystsSplitXSec(std::string systString, bool useND = true,
                        bool useFD = true, bool useNueOnE = false);

std::vector<const ana::ISyst *>
GetListOfSystsSplitXSec(char const *systCString, bool useND = true,
                        bool useFD = true, bool useNueOnE = false);
...
struct SplitXSecCorrelationConfig {
  double globalRho = 0;
  bool splitAllXSec = true;
  std::vector<std::string> splitXSecNames;
  std::vector<std::string> dontSplitXSecNames;
  std::map<std::string, double> splitXSecCorrByName;
  std::vector<std::string> splitXSecCorrNames;
  std::vector<double> splitXSecCorrStrengths;
};
