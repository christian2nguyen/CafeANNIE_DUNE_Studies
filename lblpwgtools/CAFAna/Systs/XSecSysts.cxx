...
namespace {
std::string SplitXSecName(const std::string &baseName,
                          XSecDetectorComponent detComponent) {
  switch (detComponent) {
  case XSecDetectorComponent::kNear:
    return baseName + "_ND";
  case XSecDetectorComponent::kFar:
    return baseName + "_FD";
  case XSecDetectorComponent::kBoth:
    return baseName;
  }
...
std::string SplitXSecLatexName(const std::string &baseName,
                               XSecDetectorComponent detComponent) {
  switch (detComponent) {
  case XSecDetectorComponent::kNear:
    return baseName + " (ND)";
...
void XSecSyst::Shift(double sigma, Restorer &restore, caf::StandardRecord *sr,
                     double &weight) const {
  if (fDetComponent == XSecDetectorComponent::kNear && sr->dune.isFD) {
    return;
  }
  if (fDetComponent == XSecDetectorComponent::kFar && !sr->dune.isFD) {
    return;
  }
...
XSecSyst::XSecSyst(int syst_id, const std::string &shortName,
                   const std::string &latexName, bool applyPenalty,
                   XSecDetectorComponent detComponent)
    : ISyst(shortName, latexName, applyPenalty, GetXSecSystMin(syst_id),
            GetXSecSystMax(syst_id)),
      fID(syst_id), fDetComponent(detComponent) {}
...
std::vector<const ISyst *> GetSplitXSecSysts(std::vector<std::string> names,
                                             bool applyPenalty) {
  if (names.empty()) {
    names = GetAllXSecSystNames();
  }

  static std::vector<const ISyst *> allSplitSystsPenalty;
  static std::vector<const ISyst *> allSplitSystsNoPenalty;
  std::vector<const ISyst *> &allSplitSysts =
      applyPenalty ? allSplitSystsPenalty : allSplitSystsNoPenalty;
  if (allSplitSysts.empty()) {
    for (const std::string &baseName : GetAllXSecSystNames()) {
      const int idx = GetXSecSystIndex(baseName);
      allSplitSysts.push_back(new XSecSyst(
          idx, SplitXSecName(baseName, XSecDetectorComponent::kNear),
          SplitXSecLatexName(baseName, XSecDetectorComponent::kNear),
          applyPenalty, XSecDetectorComponent::kNear));
      allSplitSysts.push_back(new XSecSyst(
          idx, SplitXSecName(baseName, XSecDetectorComponent::kFar),
          SplitXSecLatexName(baseName, XSecDetectorComponent::kFar),
          applyPenalty, XSecDetectorComponent::kFar));
    }
  }

  std::vector<const ISyst *> ret;
  for (const ISyst *s : allSplitSysts) {
    const std::string shortName = s->ShortName();
    for (const std::string &baseName : names) {
      if (shortName == SplitXSecName(baseName, XSecDetectorComponent::kNear) ||
          shortName == SplitXSecName(baseName, XSecDetectorComponent::kFar)) {
        ret.push_back(s);
        break;
      }
    }
