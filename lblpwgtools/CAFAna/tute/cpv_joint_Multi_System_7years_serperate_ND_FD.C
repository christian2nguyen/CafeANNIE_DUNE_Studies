  ...
  bool IsXSecParamName(const std::string& name)
  {
    static const std::set<std::string> xsecNames = [](){
      ...
      for(const auto& systName: ana::GetAllXSecSystNames()){
        ...
        names.insert(systName + "_ND");
        names.insert(systName + "_FD");
      }
      return names;
    }();
...
"flux_Nov17_29"
  ...
  SplitXSecCorrelationConfig xsecCorrConfig;
  xsecCorrConfig.globalRho = 0.0;
  xsecCorrConfig.splitAllXSec = true;
  xsecCorrConfig.splitXSecNames = {};
  xsecCorrConfig.dontSplitXSecNames = {
      "MaCCQE",
      "VecFFCCQEshape",
      "MaCCRES",
      "MvCCRES",
  ...
  xsecCorrConfig.splitXSecCorrByName = {
      {"C12ToAr40_2p2hScaling_nu", 0.1},
      {"C12ToAr40_2p2hScaling_nubar", 0.1},
      {"CCQEPauliSupViaKF", .3},
      {"BeRPA_A", .3},
      {"BeRPA_B", .3},
      {"BeRPA_D", .3},
  ...
  xsecCorrConfig.splitXSecCorrNames = {};
  xsecCorrConfig.splitXSecCorrStrengths = {};

  // Start from the shared xsec list and let RunFitPoint_ND_FD_Separation
  // decide which dials become ND/FD split pairs.
  ...
  if (!fitBias){
    ...
    for (auto syst : trueSyst.ActiveSysts()){
      ...
      if(std::find(ana::GetAllXSecSystNames().begin(),
                   ana::GetAllXSecSystNames().end(),
                   syst->ShortName()) != ana::GetAllXSecSystNames().end()){
        bias_syst_names.push_back(syst->ShortName() + "_ND");
        bias_syst_names.push_back(syst->ShortName() + "_FD");
      }
    }
  ...
  for (int idcp = 0; idcp < 2; ++idcp) {  
    for(int ihie = -1; ihie <= +1; ihie += 2) {
      ...
      FitTreeBlob fitBlob("fit_info", "meta_tree");
	      ...
	      thischisq = RunFitPoint_ND_FD_Separation(stateFname, sampleString,
  ...
  for(int idcp = 0; idcp < nsteps+1; ++idcp) {
	...
	SystShifts testSyst = kNoShift;
		...
		thischisq = RunFitPoint_ND_FD_Separation(stateFname, sampleString,
							 ...
							 oscVars, systlist,
							 testOsc, testSyst,
							 oscSeeds, penalty, Fitter::kNormal, nullptr, nullptr,
							 nullptr, ana::junkShifts, xsecCorrConfig);
...

    {
      ...
      FitTreeBlob fitBlob("fit_info", "meta_tree");
	      ...
	      bestChiSq = RunFitPoint_ND_FD_Separation(stateFname, sampleString,
						       trueOsc, trueSyst, false,
...
void cpv_joint_Multi_System_7years_serperate_ND_FD(){
std::string outputname = "/pnfs/dune/persistent/users/cnguyen/cafana_example/State_withsystem_set.1_and.3_only10flux_";
