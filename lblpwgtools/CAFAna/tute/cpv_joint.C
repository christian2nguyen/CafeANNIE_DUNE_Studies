...
void cpv_joint(std::string stateFname  = "/pnfs/dune/persistent/users/cnguyen/cafana_example/State"/*="/pnfs/dune/persistent/users/cnguyen/cafana_example/toy_2perEnergy_test_FD_FHC_v1.root"*//*"common_state_mcc11v3.root"*/,
	       std::string outputFname_input="State_",
	       std::string systSet = "allsyst"/*allsyst,nosyst*/, std::string sampleString="ndfd:10year",
	       std::string penaltyString="nopen", int hie=9, std::string asimov_set="0",
	       int isetCP = 0,
	       std::string fakeDataShift = "", int fitBias = 0){

std::string paraset_name = GetAsimovName(hie, 1);
std::string outputFname =  outputFname_input + "_"+ paraset_name + "_624ktmwyr_" + sampleString + "_" + systSet  + "_" + penaltyString + ".root";
 std::cout<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"<< std::endl;
