...
std::string stateFname  = "/pnfs/dune/persistent/users/cnguyen/cafana_example/State"/*="/pnfs/dune/persistent/users/cnguyen/cafana_example/toy_2perEnergy_test_FD_FHC_v1.root"*//*"common_state_mcc11v3.root"*/,
	       std::string systSet ="det:xsec" /*"det:xsec:nflux=30"*//*allsyst,nosyst, det:xsec:nflux=20*/, 
...
// Create a directory
 ...
 // 
  ...
  TDirectory* dir = fout->mkdir(("fit_glob_chisqmin"));
  dir->cd();
...
////////////////////////////////////////////////////////////
  ...
  for(double idcp = 0; idcp < nsteps+1; ++idcp) {
    ...
    TDirectory* dir_loop = fout->mkdir(("fit_" + std::to_string(idcp)).c_str());
    dir_loop->cd();
	...
	thischisq = RunFitPoint(stateFname, sampleString,
				...
				oscVars, systlist,
				testOsc, testSyst,
				oscSeeds, penalty, Fitter::kNormal, dir_loop);
...
void cpv_joint_Multi_System_CovMatrix_constuction(){
...
*/
std::vector<int>makeups{1,2};

std::cout<<"Starting Loop 10 YEAR  "<< std::endl;
for(int i = 0; i < makeups.size(); i++){
std::cout<<"Index:"<<i<<std::endl;
cpv_joint(makeups.at(i), sampleString_10, outputname);
}
