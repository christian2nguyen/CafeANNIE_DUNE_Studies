...
namespace
{
  ...
  void RenderAllScanXSecPullFromCentral(TDirectory* topDir,
                                        ...
                                        std::vector<std::string>& saved)
  {
    ...
    static const std::regex scanDirRegex("^scan_true_dcp_index_([0-9]+)_bestfit$");
    const std::string histName = "xsec_pull_from_central_prefit";
    ...
    while(TKey* key = static_cast<TKey*>(next())){
      ...
      const std::string dirName = key->GetName();
      if(!std::regex_match(dirName, scanDirRegex)) continue;

      std::unique_ptr<TObject> obj(key->ReadObj());
      TDirectory* dir = dynamic_cast<TDirectory*>(obj.get());
      if(!dir) continue;

      const std::string relDir = dirName;
      const std::string contextLabel = BuildContextLabel(dir, relDir);

      TH1* hist = static_cast<TH1*>(dir->Get(histName.c_str()));
      if(!hist) continue;
      hist->SetDirectory(nullptr);

      const std::string relPath = relDir + "/" + histName;
      const std::string outBase = JoinPath(outputDir, Sanitize(relDir + "_" + histName));
      RenderHist(hist, outBase, relPath, contextLabel, combinedPdf, combinedPdfOpen, saveExtraFiles);
...
void Print_CP_fit_hist_II(std::string inputRoot = "",
                       ...
                       bool saveExtraFiles = false)
{
  ...
  RenderAllScanXSecPullFromCentral(fin, outputDir, combinedPdf, combinedPdfOpen, saveExtraFiles, saved);
